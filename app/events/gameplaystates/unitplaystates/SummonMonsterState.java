...
public class SummonMonsterState implements IUnitPlayStates {
	...
	public void summonMonster(GameplayContext context, ActorRef out, String u_configFile, Card statsRef, Tile summonTile) {
		...
		UpdateState.updatePlayerHealth(out, context.getGameStateRef());
		UpdateState.updatePlayerMana(out, context.getGameStateRef());
		...
		UpdateState.updateUnitStats(out, summonedMonster, summonTile);
		UpdateState.threadSleep();
				...
				if(a.getActivateMoment() == ActivateMoment.Summon) {
					...
					if (a.getTargetType() == Avatar.class) {
						...
						if (a.targetEnemy() == false) {						
							targetAvatar = context.getGameStateRef().getPlayerAvatar();
						}
						else {
						...
						if (a.getEffectAnimation() != null) {
							BasicCommands.playEffectAnimation(context.out, a.getEffectAnimation(), context.getGameStateRef().getPlayerAvatar().getPosition().getTile(context.getGameStateRef().getBoard()));
							UpdateState.threadSleep();
						}
